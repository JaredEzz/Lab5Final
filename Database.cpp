    ...
    for (unsigned int i = 0; i < rules.size(); i++)
    {
        ...
        for (unsigned int j = 0; j < rules.size(); j++)
        {
            for (unsigned int k = 0; k < predList.size(); k++)
            {
                ...
                {
                    dependancyGraph.addAdjacentNode("R", i, "R", j);
                }
            }
        }
        ...
        {
            dependancyGraph.addNode("Q", j);

            if (rules.at(i).getHead().getIdentity() == queries.at(j).getIdentity())
            {
                dependancyGraph.addAdjacentNode("Q", j, "R", i);
            }
        }
...
void Database::evaluateQueries()
{
    for (unsigned int i = 0; i < queries.size(); i++)
    {
        ...
        dependancyGraph.dfs(dependancyGraph.getNode("Q", i));
